...
function setting() {
  ...
  var yu = parseInt(10 * newSpace) % parseInt(10 * oldSpace);
  if (yu) {
    showTip(
...
function changeTag(item) {
  if (activeImg.length > 0) {
    ...
    // 不完全知道做了啥，可以肯定是有 show/hide 右上角的 Delete 按钮
    toActive();
  ...
  if (!flag) {
    ...
    if (answer) {
      ...
      if (oldState == 0) {
        ...
        window.location.href = "../index.html?type=unTag";
      } else {
        localStorage.setItem(
          "tagedTotal",
          parseInt(localStorage.getItem("tagedTotal")) - 1
        ...
        localStorage.setItem("result", JSON.stringify(result));
        window.location.href = "../index.html?type=taged";
      }
    } else {
      localStorage.setItem("result", JSON.stringify(result));
    }
...
function format(arr) {
  // 输入是所有标记过的图片列表
  // [{tagId: label_id.int, tag: label.string, frameData: time.list[float]}]
  var result = [];
  if (arr.length > 0) {
    var result = [];
    var tagInfoArr = [];
    ...
    // tagInfoArr 每个元素就是每类标签相关信息，tagId/tag 表示标签本身信息，frameData是列表，表示所有相关图片的time
    // 注意：每个标签对应一个元素，每个元素中可能有不止一个样本
    tagInfoArr.push(obj);
...
function dispart(arr) {
  ...
  var start = arr[0];
  for (var i = 1; i < arr.length; i++) {
    if (Math.abs(arr[i] - start - newSpace) > 1e-5) {
      newArr.push(-1);
    }
    newArr.push(arr[i]);
    start = arr[i];
